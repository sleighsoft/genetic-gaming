...
class Car(object):
  ...
  def __init__(self, shape, position, rotation, rotation_speed, base_velocity,
               ...
               max_velocity, min_velocity, color, sensor_range, num_sensors,
               sensor_color=(0, 0, 0)):
    # Static
    self._shape = shape
  ...
  def trigger_acceleration(self, acceleration_value):
    ...
    actual_velocity = self._velocity + (self._acceleration * acceleration_value)
    self._velocity = max(self._min_velocity, min(self._max_velocity, actual_velocity))
