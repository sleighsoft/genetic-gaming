...
class Game(object):
  ...
  def reset(self):
    """Reset game state."""
    car_position = (self.SCREEN_WIDTH // 2 - 150, self.SCREEN_HEIGHT // 2)
    self.cars = [
      Car(shape=(10, 10),
  ...
  def run(self):
    ...
    # TODO playground
    def get_border(x, y, width=100, height=10):
    ...
    while True:
      ...
      # Walls
      OFFSET_LEFT = self.SCREEN_WIDTH // 2 - 200
      # Horizontal Corridor
      self.screen.fill(border_color, get_border(OFFSET_LEFT, self.SCREEN_HEIGHT // 2 - 50, width=300))
      self.screen.fill(border_color, get_border(OFFSET_LEFT, self.SCREEN_HEIGHT // 2 + 50, width=450))
      self.screen.fill(border_color, get_border(OFFSET_LEFT, self.SCREEN_HEIGHT // 2 - 50, width=10, height=100))

      # Vertical Corridor
      OFFSET_LEFT2 = self.SCREEN_WIDTH // 2 + 100
      self.screen.fill(border_color, get_border(OFFSET_LEFT2, 10, width=10, height=190))
      self.screen.fill(border_color, get_border(OFFSET_LEFT2 + 150, 10, width=10, height=290))
      self.screen.fill(border_color, get_border(OFFSET_LEFT2, 10, width=150, height=10))
