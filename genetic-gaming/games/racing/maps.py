...
def intersect(p0, p1, p2, p3):
  ...
  return intersect_point is not None and \
         intersect_point not in [p0, p1, p2, p3]
...
class MapGenerator(object):
  ...
  def generate(self):
    ...
    next_left, next_right, center = self.generate_first_segment(
        last_left, last_right, Vec2d(self._start_point))
    found = [self.get_wall(last_left, last_right),
             self.get_wall(last_left, next_left),
             self.get_wall(last_right, next_right)]
    last_left = next_left
    ...
    while tries_left > 0:
      ...
      if self.is_valid(next_left) and self.is_valid(next_right) and \
         self.is_valid_line(last_left, next_left, found) and \
         self.is_valid_line(last_right, next_right, found):
